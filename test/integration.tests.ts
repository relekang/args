...
import { Config, SingleCommandConfig } from '../src/types';
...
describe('single-command', () => {
  const config: SingleCommandConfig = {
    name: 'supercli',
    single: true,
    help: 'Testing testing',
    manual:
  ...
  test('args should parse the positional options', async () => {
    await args(config)(['node', 'cli', 'first-argument', '200']);

    expect(config.run).toBeCalledWith({
      _: ['first-argument', '200'],
      first: 'first-argument',
      second: 200,
    });
  ...
  test('args should parse the named option', async () => {
    await args(config)(['node', 'cli', 'first-argument', '--verbose']);
